...
class NormalizeAndPad(object):
    def __init__(self, model_name, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], size=640,
                 aug_translate=False):
        self.mean = mean
        self.std = std
        self.size = size
    ...
    def __call__(self, input_dict):
        ...
        if 'box' in input_dict:
            ...
            input_dict['box'] = box
        img = FF.interpolate(img.unsqueeze(0), size=(self.size, self.size), mode='bilinear',
                             align_corners=True).squeeze(0)
        if 'gt_mask' in input_dict:
            gt_mask = input_dict['gt_mask']
            gt_mask = FF.interpolate(gt_mask.unsqueeze(0).unsqueeze(0).float(), size=(self.size, self.size),
